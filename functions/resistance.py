...
def resistance(lwl, bwl, tc, alcb, cp, cm, awp, disp, lcb, lcf, vboat, heel):
    ...
    Fn = vboat/(lwl*grav)**0.5      # Frounde number
    
    # 1 RESISTENCIA VISCOSA
    Recb = (0.7*lwl*(vboat))/ni
    Cfcb = (0.075/((np.log(Recb)/np.log(10))-2)**2)-(1800/Recb)
    kcb = 0.09  # initial guess for the centre of gravity
    scb = (1.97+0.171*(bwl/tc))*((0.65/cm)**(1/3))*(disp*lwl)**0.5  # estimation for the canoe body surface area
    ...
    for k in range(1, 10, 1):
        if float(coefficient_delft[k][0]) <= Fn1 and Fn1 <= float(coefficient_delft[k + 1][0]):
            XX = [float(coefficient_delft[k][0]), float(coefficient_delft[k + 1][0])]
            for j in range(1, 10, 1):
                YY = [float(coefficient_delft[k][j]), float(coefficient_delft[k + 1][j])]
    ...
    for k in range(1, 7, 1):
        if float(coefficient_inclination[k][0]) <= Fn2 and Fn2 <= float(coefficient_inclination[k + 1][0]):
            SS = [float(coefficient_inclination[k][0]), float(coefficient_inclination[k + 1][0])]
            for j in range(1, 7, 1):
                TT = [float(coefficient_inclination[k][j]), float(coefficient_inclination[k + 1][j])]
    ...
    for k in range(1, 7, 1):
        if float(coefficient_viscous[k][0]) <= heel2 and heel2 <= float(coefficient_viscous[k + 1][0]):
            UU = [float(coefficient_viscous[k][0]), float(coefficient_viscous[k + 1][0])]
            for j in range(1, 5, 1):
                VV = [float(coefficient_viscous[k][j]), float(coefficient_viscous[k + 1][j])]
