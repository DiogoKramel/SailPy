    ...
    for t in range (0,len_veltw,1):    #para otimizacao, o for t é identado juntamente com bwl 
        for u in range (0,len_betatw,1):
            def f(y):
                ...
                leeway=y[0]     #declarar as quatro variaveis que resolvem o problema
                vboat=y[1]
                heel=y[2]
                deltaR=y[3]
                ...
                alfaR=0
                arcsinvalue=-np.cos(heel)*np.sin(deltaR)+np.sin(leeway)*np.cos(deltaR)*np.cos(heel)
                if arcsinvalue < (-1):
                    alfaR=np.arcsin(-0.9)
                elif arcsinvalue > (1):
                    alfaR=np.arcsin(0.9)
                else:
                    alfaR=np.arcsin(arcsinvalue)
                ...
                elif marcaS==2:       #main and spinnaker
                    #Mestra
                    Am=P*E/2
                    CEm=(0.39*P)+BAD
                    #Genoa
                ...
                elif marcaS==3:       #both combinations
                    #Mestra
                    Am=P*E/2
                    CEm=(0.39*P)+BAD
                    #Genoa
                ...
                #D_S=[-abs(DS*np.cos(heel)*np.cos(betaaw)),np.sign(alfa_aero)*(DS*np.cos(heel)*np.sin(betaaw)),-abs(DS*np.sin(heel))]

                ### 2.7 Equacoes de equilibrio sistema de referencia na linha dagua
                #as componentes individuais sao descritas abaixo mas elas sao diretamente aplicadas na equacao de F
                #na direcao x
                #dx=(LS_B0[0]+Lk_b0[0]-Rt+D_S[0])
                #na direcao y
                #dy=LS_B0[1]+Lk_b0[1]+D_S[1]
                #Lkell=Lk_b0[1]      #lift keel
                #Lsail=LS_B0[1]      #lift sail
                #DragS=D_S[1]        #drag sail
                #em torno de x
                #dmx=(LS_B0[1]+D_S[1])*(CEa+freeb)+(-CEh)*Lk_b0[1]+Mr
                ZCEa=CEa+fb
                #Fabs=(float(dx)**2+float(dy)**2+float(dmx)**2)**(0.5)      #F absolute magnitude
                ...
                #count=count+1   #contador de iteracoes dentro do vpp
                return np.array([np.float(Fx),np.float(Fy),np.float(Mx),np.float(Mz)])

            x0=initialguess                             #initial guess to leeway, vboat, heel, deltaR
            sol=optimize.root(f,x0,method='hybr')       #solve for Powell hybrid method (hybr) ou Levenber-Marquart (lb)

            ### 2.8 Output data
            vboatfinal[t,u]=vboat
            vmgfinal[t,u]=vmg
