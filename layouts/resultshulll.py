...
resultshull = dbc.Container([
    dbc.Col([
        ...
        dbc.Row([
            dbc.Col([
                html.P("The main results are displayed below. First, you can view the evolutionary process in the first main plot. Colors will indicate how the individual evolved towards the optimal solution after each new generation. When hovering each individual, you can see the plot of each resistance component updating, as well a representation of the hull sections. The data includes also individuals that are not valid, and the constraints disrespected can be seen in the 'Constraints' menu below. The same plot also allows visualizing and understanding the Pareto frontier, in which further improvement is not possible."),
            ], className = 'justify'),
            dbc.Col([
                html.P("Besides that, a list with all the individuals generated is displayed in a comprehensive dynamic table. In this table, the user can sort the values combining ascending and descending sorting in each column. Another import asset is the parallel dimensions plot. It shows how the dimension set of the hull throughout the optimization process. The concentration at one of the extremes may indicate space for further exploration and also a trend to what values give the best results."),
            ], className = 'justify'),
        ]),
