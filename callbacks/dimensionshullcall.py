...
@app.callback(Output('output-feasibility', 'children'), [Input('cwp', 'value'), Input('bwl', 'value'), Input('lwl', 'value'), Input('cb', 'value'), Input('tc', 'value'), Input('cm', 'value')])
def feasibility(cwp, bwl, lwl, cb, tc, cm):
    lwl = np.float(lwl)
    bwl = np.float(bwl)
    tc = np.float(tc)
    cb = np.float(cb)
    ...
    if prismatic > 0.6 or prismatic < 0.52 or lwlbwl > 5 or lwlbwl < 2.73 or bwltc > 19.38 or bwltc < 2.46 or lwldisp > 8.5 or lwldisp < 4.34 or loadingfactor > 12.67 or loadingfactor < 3.78:
        return dbc.Alert("The study sailboat may not be feasible. Consider changing the dimensions.", color="danger", style={'padding': '5pt', 'display': 'inline-block'}),
    else:
...
def other_dimensions(lwl, bwl, cb, cwp, lcf, lcb, tc, cm, beta_n):
    ...
    '''
    return 'Displacement: {} m3'.format(round(disp,2)), html.Br(), 'Waterplane Area: {} m2'.format(round(awp,2)), html.Br(), 'Canoe Body Lateral Area: {} m2'.format(round(disp,2)), html.Br(), 'Canoe Body Surface Area: {} m2'.format(round(disp,2))