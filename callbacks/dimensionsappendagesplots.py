...
def create(sailset, tc, lwl, disp, bwl, lcb, lcf, overhang, bowangle, freeboard, poskeel, sweepkeel, spankeel, tipchordkeel, rootchordkeel, heightsurfacerudder, spanrudder, rootchordrudder, tipchordrudder, sweeprudder, posrudder, mastdiameter, mastheight, boomheight, psail, esail, isail, jsail, mastpos, boa, rootchordkeeltcks, tipchordkeeltcks, rootchordruddertcks, tipchordruddertcks, mzncheck, pmz, emz, badmz, spl, lpg, crewmass, keelnaca, ruddernaca):
    ...
    deckx1 = -overhang
    decky1 = np.float(overhang)*(np.float(tc)/(np.float(lwl)/5))
    deckx2 = deckx1*0.9
    decky2 = freeboard*0.6
    deckx3 = 0
    decky3 = freeboard*0.9
    deckx4 = (np.tan(bowangle)*freeboard)+np.float(lwl)
