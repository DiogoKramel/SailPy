    ...
    return {
        'data': [
            go.Parcoords(
                ...
                dimensions = list([
                    dict(label = 'Waterline Length', values = df['LWL']),
                    dict(label = 'Waterline Beam', values = df['BWL']),
                    dict(label = 'Draft', values = df['Draft']),
                    dict(label = 'Displacement', values = df['Displacement']),
                    dict(label = 'Waterplane area', values = df['AWP']),
                    dict(label = 'LCB', values = df['LCB']),
                    dict(label = 'LCF', values = df['LCF']),
                ]),
            ),
        ],
        'layout': go.Layout(
            title="Each individual dimensions",
            hovermode="closest",
            margin={
                "r": 20,
                "t": 100,
