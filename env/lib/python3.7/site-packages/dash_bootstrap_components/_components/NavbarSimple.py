...
otherwise.
- fluid (boolean; optional): The contents of the Navbar are wrapped in a container, use fluid=True to
make this container fluid, so that in particular, the contents of the
navbar fill the available horizontal space.
- light (boolean; optional): Applies the `navbar-light` class to the NavbarSimple, causing text in the
...
- expand (boolean | string; optional): Specify breakpoint at which to expand the menu bar. Options are: 'xs',
